...
class HttpSrv {
    ...
    setStatic(url, folder, defaultFile) {
        ...
        this.onGet(regExp, (req, res) => {
            ...
            const fileFullName = Path.join(appDir, folder, fileName);
     ...
     * @property {object} [httpSrv=new HttpSrv()] HttpSrv object
     * @property {string} [address='127.0.0.1'] IP address of server
     * @property {number} [port=1337] Port number
     */
    /**
     * Runs HTTP server
     * @param {ServerConfig} [config={}] Http Server configuration object
     * @return {object} return HttpSrv object
     */
    static run(config = {}) {
        const { httpSrv = new HttpSrv(), address = '127.0.0.1', port = 1337 } = config;
